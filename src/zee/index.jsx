...
import { useGetSingleMealsId } from "./hook/useGetSingleMealsId";
...
const HelloFoodies = () => {
    ...
    const response = locatoin?.state?.res

    const { FoodSelectId } = useGetSingleMealsId()
        ...
        <div className="">
            <Link to={'/cart'}>
                <div className="fixed top-0 left-0 z-50 ml-5 flex items-center justify-center w-12 h-12 bg-gray-00 text-white rounded-full shadow-md">
                    ...
                    <span className="absolute top-0 right-0 -mt-1 -mr-1 w-6 px-2 py-1 bg-red-500 text-white text-xs rounded-full">{items?.result[0]?.items?.length}</span>
                </div>
            </Link>
            <div className="bg-gray-100 min-h-screen py-8 dark:bg-gray-900">
                                ...
                                return (
                                    <div key={i} className="grid grid-rows-12 relative w-full bg-white rounded-xl shadow-lg overflow-hidden h-[430px] dark:bg-gray-950">
                                        ...
                                        <div className="p-4 row-span-3 self-end">
                                            <Button onClick={() => FoodSelectId(el?._id)} className="w-full text-white bg-indigo-600 hover:bg-indigo-700 mt-3 dark:text-white dark:bg-indigo-600">View Details</Button>
                                        </div>
                                    </div>
                                )
                            })
